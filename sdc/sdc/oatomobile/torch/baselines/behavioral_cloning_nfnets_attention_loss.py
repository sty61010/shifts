...
from sdc.oatomobile.torch.networks.perception_nfnets_attention \
import NFNets_Attention
...
class BehaviouralModel_NFNets_Attention_Loss(nn.Module):
        ...
        Args:
            ...
            generation_mode: one of {sampling, teacher-forcing}.
                ...
                In the latter case, the likelihood is formed by conditioning
                    on ground-truth.
        """
        super(BehaviouralModel_NFNets_Attention_Loss, self).__init__()
        assert generation_mode in {'sampling', 'teacher-forcing'}

        self._output_shape = output_shape

    # The convolutional encoder model.
        self._encoder = NFNets_Attention(
        ...
        if bc_deterministic:
            ...
            print('BC NFNets Attention Model with ADE Loss: using Gaussian likelihood.')
        ...
        print(f'BC NFNets Attention Model with ADE Loss: using generation mode {generation_mode}.')
...
#         print("loc: ", torch.stack(dlocs, dim=1).reshape(y.shape[0], -1))
...
def train_step_bc_nfnets_attention_loss(
    model: BehaviouralModel_NFNets_Attention_Loss,
...
#     print("trajectoy.shape", batch.get("ground_truth_trajectory").shape)
#     print("feature_map.shape", batch.get("feature_maps").shape)
    ...
    if model.bc_deterministic:
        ...
        # add all loss
        loss = nll_w*nll + ade_w*ade + fde_w*fde
...
def evaluate_step_bc_nfnets_attention_loss(
    model: BehaviouralModel_NFNets_Attention_Loss,
    batch: Mapping[str, torch.Tensor],
) -> Mapping[str, torch.Tensor]:
